...
class DataImportModelLinkageTests(TestCase):
    ...
    def test_host_and_pathogen_linkage_with_id_reassignment(self):
        ...
        study = models.Descriptive.objects.create(
            id=100, original_id="study_orig", full_text=None
        )
        ...
        fake_species = SimpleNamespace(
            ...
            name_lookup=lambda q, limit=5: {
                "results": [{"taxonomicStatus": "ACCEPTED", "canonicalName": q}]
            },
        )
...
class ExcelImportTests(TestCase):
    ...
    def test_import_from_excel(self):
        ...
        self.assertTrue(
            os.path.exists(test_xlsx_path),
            f"test.xlsx not found at {test_xlsx_path}; create the file with the expected sheets to run this test.",
        )
        ...
        ft = models.FullText.objects.filter(
        ...
        desc = models.Descriptive.objects.filter(
            dataset_name="Pygmy rice rat as potential host of Castelo dos Sonhos Hantavirus."
        ).first()
        ...
        host = models.Host.objects.filter(
            scientific_name="Olygoryzomys utiaritensis"
        ).first()
        self.assertIsNone(host)
        host = models.Host.objects.filter(
            scientific_name="Oligoryzomys utiaritensis"
        ).first()
